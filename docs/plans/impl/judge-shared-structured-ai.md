...
- [x] Add a companion table keyed by `claim_key_hash` with a foreign key to
      ...
      rebuild 0010.
- [x] Append 0011 to `EXPECTED_D1_MIGRATIONS`.
- [x] Re-run the exact contract command. Expected GREEN: all migration tests
      pass, including second application.
- [x] Run `npm run typecheck`.
- [x] Commit: `feat: add durable judge claim lifecycle`
...
- [x] Define the additive API in the test:
...
- [x] Implement strict validation and single conditional D1 statements.
      ...
      release delete.
- [x] Re-run the exact Cloudflare command. Expected GREEN.
- [x] Run `npm run lint && npm run typecheck`.
- [x] Commit: `feat: add judge claim state transitions`
...
- [x] Fault-inject the uncertainty case after the D1 insert commits but before
...
- [x] Define the concrete additive API:
...
- [x] Refactor `reserve()` to generate an ID and delegate to
...
- [x] Re-run the exact Cloudflare command. Expected GREEN.
- [x] Run `npm run lint && npm run typecheck`.
- [x] Commit: `feat: recover named judge reservations`
...
- [x] Add RED assertions to each successful adapter test:
...
- [x] Add one retry test per Decision/invalidation adapter where the first
      response has trustworthy usage but invalid output. Expect two attempt
      rows and summed totals.
- [x] Add table tests for missing model, missing usage, negative/fractional/
...
- [x] Implement `StructuredAiBilling` and a reusable accumulator with
      `record(responseModel, usage)`, `invalidate()`, and
      `complete(attemptCount)`. Keep `PrivateDisclosureBilling` as an alias.
- [x] Add optional billing only to concrete adapter result types. Do not change
      application interfaces, protocol schemas, or events.
- [x] Re-run the exact Vitest command. Expected GREEN.
- [x] Run `npm run lint && npm run typecheck`.
- [x] Commit: `feat: meter all structured AI responses`
...
- [x] Add RED jitter tests with injected random `0.5`. First retry expects
...
- [x] Add RED descriptor tests for exact Decision/invalidation caps, envelopes,
      costs, two attempts, 120-second lease, 25-hour retention, eight
      generations, and 2,171,200 tokens.
- [x] Add RED pricing tests for each supported GPT-5.6 model, mixed response
      models, per-attempt long context, unknown/versioned model, unsafe
      arithmetic, and usage outside each envelope.
- [x] Add RED `judge-structured-input-v1` tests. Equal entity sets in different
...
- [x] Implement the fixed full-jitter formula, descriptor constants, generic
...
- [x] Include `judge-structured-input-v1` in every structured pricing version.
- [x] Re-run the exact Vitest command. Expected GREEN.
- [x] Run `npm run lint && npm run typecheck`.
- [x] Commit: `feat: bound shared judge AI operations`
...
- [x] Add RED sink-privacy tests. Raw content is allowed in transient
...
- [x] Implement `JudgeManagedStructuredAiError` and the generic lifecycle.
      Provider invocation must follow a successful generation-bound
      `markProviderStarted()` result.
- [x] Refactor private disclosure to use the generic lifecycle. Remove use of
      the old claim/release path only after equivalent unit tests are green.
- [x] Re-run the exact Vitest command. Expected GREEN.
- [x] Run the Task 2 and Task 3 Cloudflare tests to confirm lifecycle/named
      reservation compatibility.
- [x] Run `npm run lint && npm run typecheck`.
- [x] Request spec review, then quality review; resolve all
      Critical/Important findings.
- [x] Commit: `refactor: unify judge structured AI lifecycle`
...
- [x] Add RED D1 repository tests for
...
- [x] Add RED unit tests for a 20-row batch ordered by stale timestamp/hash.
...
- [x] Add RED command contract tests. The command accepts only target and
...
- [x] Implement bounded automatic reconciliation before managed operations.
...
- [x] Implement `npm run judge:reconcile -- <target> --dry-run|--apply`.
...
- [x] Write the exact runbook for legacy verification, full-finalization,
      25-hour retention, partial failure, and escalation. It forbids releasing
      provider-started usage.
- [x] Re-run the exact unit/contract commands. Expected GREEN.
- [x] Run:
...
- [x] Run `npm run security:secrets && npm run lint && npm run typecheck`.
- [x] Commit: `feat: reconcile durable judge AI work`
...
- [x] Add RED decorator tests for completed application replay before claim,
...
- [x] Assert stored pricing version and request fingerprint both change when
      only `judge-structured-input-v1` changes.
- [x] Add RED HTTP tests for judge `ai_preferred`, ordinary denial, manual zero
...
- [x] Implement the decorator only after current session, assignment,
...
- [x] Keep manual dependencies synthesizer-free and deterministic dependencies
      outside the managed lifecycle.
- [x] Re-run the exact Vitest command plus
      `tests/unit/application/decision-candidates.test.ts`. Expected GREEN.
- [x] Run `npm run lint && npm run typecheck`.
- [x] Request spec review, then quality review; resolve all findings.
- [x] Commit: `feat: meter judge Decision synthesis`
...
- [x] Add RED decorator tests for original event/revision claim identity,
...
- [x] Assert the canonicalization version is present in pricing metadata and
      changes the fingerprint.
- [x] Add RED HTTP tests where the external receipt is durable before:
...
- [x] Assert ordinary/deterministic paths create no judge claim/ledger and
...
- [x] Construct the evaluator decorator only after current authorization.
...
- [x] Run `npm run lint && npm run typecheck`.
- [x] Request spec review, then quality review; resolve all findings.
- [x] Commit: `feat: meter judge invalidation evaluation`
...
- [x] Add RED Cloudflare cases with injected concrete fakes for all three
...
- [x] Add RED gate/config cases requiring exact gate, allowlisted judge, judge
...
- [x] Construct operation-specific concrete adapters only in the configured
      request-scoped runtime. Default deployed code contains no fake.
- [x] Run `npm run cloudflare:types` to generate bindings; do not hand-edit the
      generated file.
- [x] Re-run the exact Cloudflare/contract commands. Expected GREEN.
- [x] Run:
      `npm run cloudflare:types:check && npm run cloudflare:config:check &&
      npm run lint && npm run typecheck`.
- [x] Request spec review, then quality review; resolve all findings.
- [x] Commit: `feat: gate all judge structured AI work`
...
- [x] Add a Node browser UI-contract test using route interception for
      invalidation 429 and 202 pending. Assert the error/pending state does not
      clear durable meeting text or manual controls.
- [x] Add a Worker browser test against real Wrangler with managed provider
...
- [x] If no UI file changes, retain no regenerated screenshots/clips. If a UI
      file changes, add E2E capture and synthetic screenshots per `AGENTS.md`.
- [x] Commit: `test: cover bounded judge AI degradation`
...
- [x] Request final whole-range spec review, then quality review. Resolve every
      Critical/Important finding and rerun affected commands.
- [x] Update Plan 05 and `_status.md`. Close only local claim/reservation/check/
...
- [x] Mark only proven plan checkboxes complete.
- [x] Run `git diff --check` and verify the worktree contains only reviewed
      files.
- [x] Commit: `docs: close durable judge structured AI billing`
