...
| AC-12 | Local proven | `npm run compose:smoke` creates a new 3-user meeting, uploads source/derived artifact data, commits Decision revisions 1/2, recreates the app container, reuses the same session for API verification, then opens the persisted meeting/artifact/Evidence/Decision through headless Playwright | Hosted D1/DO migration and deployment proof                                                                       |
