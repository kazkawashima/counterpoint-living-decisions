...
- judge modeはUSD 25のrolling費用上限で新規server-funded workを止める。
...
- key sourceを`facilitatorProvided`、`judgeManaged`、`judgeProvided`で明示的に分ける。
- `judgeManaged`ではDOにstandard keyを渡さない。
- allowlist済みjudgeの`judgeProvided`はタブから認証済みWorkerへ短命secret発行時だけ
  送信し、Workerは保存・返却・ログ出力しない。server-funded judgeは引き続きBYOK不要。
